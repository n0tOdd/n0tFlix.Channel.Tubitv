...
namespace n0tFlix.Channel.TubiTV
{
    ...
    {
        public static async Task<ChannelItemResult> GetGenres(ILogger logger)
        {
            ...
            result.Items.Add(new ChannelItemInfo()
            {
                Id = "Foreign_Language_Films".ToLower(),
        ...
        public static async Task<ChannelItemResult> CollecEpisodeItemsAsync(string GenreId, ILogger logger, IMemoryCache memoryCache)
        {
            string[] tmp = GenreId.Split("-");
            string id = tmp.Last();
            if (memoryCache.TryGetValue("tubitv-episodes-" + id, out ChannelItemResult cachedValue))
            {
                logger.LogInformation("Function={function} FolderId={folderId} Cache Hit", nameof(CollectGenreItemsAsync), "tubitv-episodes-" + id);
                return cachedValue;
            }
            ChannelItemResult result = new ChannelItemResult();
        ...
        public static async Task<ChannelItemResult> CollecSeasonItemsAsync(string GenreId, ILogger logger, IMemoryCache memoryCache)
        {
        ...
        public static async Task<ChannelItemResult> CollectGenreItemsAsync(string GenreId, ILogger logger, IMemoryCache memoryCache)
        {
        ...
        public static async Task<IEnumerable<ChannelItemInfo>> CollectLatestadded(ILogger logger, IMemoryCache memoryCache)
        {
            ...
            try
            {
                ...
                HttpClient httpClient = new HttpClient();
                string json = await httpClient.GetStringAsync("https://tubitv.com/oz/containers/recently_added/content?parentId=&cursor=120&limit=60&isKidsModeEnabled=false&expand=0");
                dynamic test = JObject.Parse(json);
                List<GenreItems> list = new List<GenreItems>();
                ...
                foreach (dynamic jObject in test.contents)
                {
                    try
                    {
                        ...
                        GenreItems item = new GenreItems(id, jObject.Value.type, title, decription, language, Tags, Directors, Actors, PosterArt, thumbnails);
                        list.Add(item);
                    }
                    catch (Exception e)
                    {
                ...
                List<ChannelItemInfo> li = new List<ChannelItemInfo>();
                foreach (GenreItems it in list)
                {
                    if (string.Equals(it.type, "v", StringComparison.OrdinalIgnoreCase))
                    {
                        li.Add(new ChannelItemInfo()
                        {
                            Id = it.id,
                            Name = it.title,
                            ...
                            ImageUrl = it.Thumbnail.FirstOrDefault(x => !string.IsNullOrEmpty(x)),
                            Type = ChannelItemType.Media,
                ...
                memoryCache.Set("tubitv-latest", li, DateTimeOffset.Now.AddDays(1));
                return li;
            }
            catch (Exception ex)
            {
